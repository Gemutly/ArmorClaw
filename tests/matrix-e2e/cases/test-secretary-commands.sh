...
echo "=== test-secretary-commands: !secretary list/status/run ==="

declare -a CMDS=(
  "!secretary list|!secretary list|secretary"
  "!secretary status|!secretary status|secretary"
  "!secretary run|!secretary run test-task|secretary"
)
...
# 2. Create room
ROOM_ID=$(matrix_create_room "$TOKEN" "secretary-test-$$") || { echo -e "${RED}FAIL: create_room${NC}"; exit 1; }
...
TOTAL=${#CMDS[@]}
echo ""
echo "--- Secretary command results: $RESPONDED/$TOTAL responded, $SKIPPED skipped ---"

if [ "$RESPONDED" -eq 0 ]; then
  echo "SKIP: !secretary commands not responding — bridge may not have secretary module enabled"
  exit 1
fi

echo -e "${GREEN}PASS: at least one !secretary command got a response ($RESPONDED/$TOTAL)${NC}"
exit 0
