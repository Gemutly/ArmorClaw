...
echo "=== test-agent-forget-skill: !agent forget-skill command ==="
...
# 2. Create room
ROOM_ID=$(matrix_create_room "$TOKEN" "forget-skill-test-$$") || { echo -e "${RED}FAIL: create_room${NC}"; exit 1; }
...
# 5. Send !agent forget-skill
EVENT_ID=$(matrix_send "$TOKEN" "$ROOM_ID" "!agent forget-skill test-agent-1 ls_xxx_123") || { echo -e "${RED}FAIL: send !agent forget-skill${NC}"; exit 1; }
echo -e "${GREEN}Sent !agent forget-skill test-agent-1 ls_xxx_123 (event: $EVENT_ID)${NC}"

# 6. Poll for any m.notice (confirmation or "not found")
...
if [[ -n "$body" ]]; then
  echo -e "${GREEN}PASS: !agent forget-skill E2E test — received notice: ${body:0:80}${NC}"
  exit 0
fi
