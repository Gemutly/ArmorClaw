...
> **Socket path mismatch**: The Python sidecar's default socket path (`/run/armorclaw/sidecar-office.sock` in `worker.py`) differs from the Go client's expected path (`/run/armorclaw/office-sidecar/sidecar-office.sock` in `office_client.go`). This is a potential runtime mismatch. Ensure `SIDECAR_SOCKET` is set explicitly or the Go client's `OfficeSocketPath` is aligned.
